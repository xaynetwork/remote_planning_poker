...
async fn main() {
    ...
    let app_state = Arc::new(AppState { games, channels });
...
async fn handle_socket(socket: WebSocket, state: Arc<AppState>, game_id: GameId) {
    ...
    let api_response = if let Some(game) = state.games.lock().unwrap().get(&game_id) {
    ...
    // TODO: it compains that the future is not `Send` as this value is used across an await
